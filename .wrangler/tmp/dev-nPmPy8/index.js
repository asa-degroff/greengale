...
// workers/firehose/index.ts
import { DurableObject } from "cloudflare:workers";
var BLOG_COLLECTIONS = [
  "com.whtwnd.blog.entry",
  "app.greengale.blog.entry"
];
var FirehoseConsumer = class extends DurableObject {
  static {
    __name(this, "FirehoseConsumer");
  }
  ws = null;
  connected = false;
  cursor = 0;
  ...
  async start() {
    ...
    const stored = await this.ctx.storage.get("cursor");
    if (stored) {
      this.cursor = stored;
    }
    await this.connect();
  ...
  async indexPost(uri, did, rkey, source, record) {
    try {
      const title = record?.title || null;
      const subtitle = record?.subtitle || null;
      ...
      const themePreset = typeof record?.theme === "object" && record.theme !== null ? record.theme.preset : record?.theme || null;
      const contentPreview = content.replace(/[#*`\[\]()!]/g, "").replace(/\n+/g, " ").slice(0, 300);
...
app.get("/xrpc/app.greengale.feed.getRecentPosts", async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      cursor: hasMore ? returnPosts[returnPosts.length - 1].indexed_at : void 0
    });
  } catch (error) {
...
app.get("/xrpc/app.greengale.feed.getAuthorPosts", async (c) => {
  ...
  try {
    ...
    if (!author.startsWith("did:")) {
      ...
      if (!authorRow) {
        return c.json({ posts: [], cursor: void 0 });
      }
      authorDid = authorRow.did;
    }
    let query = `
      SELECT
        p.uri, p.author_did, p.rkey, p.title, p.subtitle, p.source,
...
app.get("/xrpc/app.greengale.actor.getProfile", async (c) => {
  ...
  try {
    const query = author.startsWith("did:") ? "SELECT * FROM authors WHERE did = ?" : "SELECT * FROM authors WHERE handle = ?";
...
// .wrangler/tmp/bundle-3zs8Vq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3zs8Vq/middleware-loader.entry.ts
